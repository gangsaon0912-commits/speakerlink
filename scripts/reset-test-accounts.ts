...
async function resetTestAccounts() {
  ...
  const testEmails = ['instructor@test.com', 'company@test.com', 'admin@test.com']
  ...
  for (const email of testEmails) {
    try {
      ...
      if (!user) {
        console.log(`ℹ️ ${email} 계정이 존재하지 않습니다.`)
        continue
      }
      ...
      if (deleteError) {
        console.error(`❌ ${email} 계정 삭제 실패:`, deleteError.message)
      } else {
        console.log(`✅ ${email} 계정 삭제 완료`)
      }

    } catch (error) {
      console.error(`❌ ${email} 계정 삭제 중 오류:`, error)
    }
  }
...
// 스크립트 실행
resetTestAccounts().catch(console.error)
